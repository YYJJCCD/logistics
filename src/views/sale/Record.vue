<template>
  <div>
    <a-table :loading="loading" :columns="columns" :data-source="data" rowKey="id">
      <a slot="companyName" slot-scope="companyName">{{ companyName }}</a>
...
<script>
import {findAll, save} from "@/api/sale";
...
export default {
  ...
  methods: {
    ...
    confirm(record, index) {
      ...
      this.$confirm({
        title: '销售结款',
        content: '我已确定' + record.companyName + '的销售金额 ¥' + record.price + '已经打入账户!',
        okText: '确认',
        cancelText: '取消',
        onOk() {
          that.data[index].pay = true
          console.log(that.data)
          save(that.data[index]).then((res) => {
