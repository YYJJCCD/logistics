<template>
    <div class="main">
        ...
        <div style="display: flex">
            ...
            <a-button class="editable-btn" @click="getRecord">
                <a-icon type="retweet" />
                出入库记录 Excel
            </a-button>
        </div>

        <a-table bordered :loading="loading" :columns="columns" :data-source="data" rowKey="commodityId">
        ...
        <a-modal title="出入库记录" width="80%" :visible="recordVisible" :footer="null" @cancel="recordVisible = false">
            <InventoryRecords :data="recordData" />
        </a-modal>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            loadData() {
                this.loading = true
                inventoryApi.findAllByStorageId(this.$route.params.id).then((res) => {
                    console.log(res) 
                    setTimeout(() => {
                        this.data = res.data
                        this.loading = false
            ...
            submit() {
                this.form.commodityId = this.commodityList[this.selectIndex].id
                this.form.commodityName = this.commodityList[this.selectIndex].name
                inventoryApi.InAndOut(this.submitType, this.form).then((res) => {
            ...
            async getRecord(){
                ...
                this.recordData = (await findAllRecoedByStorageId(this.id)).data
            }
        }
